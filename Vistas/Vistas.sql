...
--VIEW DE DIRECTORES
CREATE OR REPLACE VIEW VW_DIRECTOR 
AS
	SELECT IDDIRECTOR,NOMBRED,APELLIDOPD,APELLIDOMD,GRADO 
    FROM DIRECTOR;

--VIEW DE EJEMPLARES
CREATE OR REPLACE VIEW VW_EJEMPLAR 
AS
	SELECT NOEJEMPLAR,IDMATERIAL,ESTATUS
	FROM EJEMPLAR;

--VIEW DE LECTORES
CREATE OR REPLACE VIEW VW_LECTOR 
AS
...
--VIEW DE LIBROS
CREATE OR REPLACE VIEW VW_LIBRO 
AS
	SELECT IDMATERIAL,NOADQUISICION,ISBN,TEMA,EDICION
	FROM LIBRO;

--VIEW DE MATERIALES
CREATE OR REPLACE VIEW VW_MATERIAL 
AS
	SELECT IDMATERIAL,TITULO,UBICACION,COLOCACION,TIPOMATERIAL 
	FROM MATERIAL;

--VIEW DE PERTENECE
CREATE OR REPLACE VIEW VW_PERTENECE 
AS
	SELECT IDAUTOR,IDMATERIAL
    FROM PERTENECE;

--VIEW DE PRESTAMOS
CREATE OR REPLACE VIEW VW_PRESTAMO 
AS
	SELECT IDLECTOR,NOEJEMPLAR,IDMATERIAL,FECHAPREST,
	FECHAVENC,NUMREFRENDO
    FROM PRESTAMO;

--VIEW DE MULTAS
CREATE OR REPLACE VIEW VW_MULTAS 
AS
	SELECT *
    FROM PRESTAMO;

--VIEW DE TESIS
CREATE OR REPLACE VIEW VW_TESIS 
AS
	SELECT IDMATERIAL,IDDIRECTOR,IDTESIS,CARRERA,ANOPUB
    FROM TESIS;

--VIEW DE TIPOLECTOR
CREATE OR REPLACE VIEW VW_TIPOLECTOR 
AS
