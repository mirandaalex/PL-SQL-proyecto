...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('AUTOR ELIMINADO EXITOSAMENTE');
END;
/
...
BEGIN    
    ...
    vDISP:= ft_prestamodisp(VIDMATERIAL,VNOEJEMPLAR);--revisa que no este disponible 
    vTotal:= ft_VERFICAMULTA(vIDLECTOR); --revisa si hay multas por crear
    vTotal:= FT_TOTALMULTAS(vIDLECTOR); --ve el total de las multas
    IF vRESTANTES>0 AND vdisp= 'DISPONIBLE' AND vTotal = 0 THEN
    ...
    ELSIF vdisp= 'DISPONIBLE' AND vrestantes=0 AND vTOtal = 0 THEN
        DBMS_OUTPUT.PUT_LINE('NO SE PUDO REALIZAR EL PRESTAMO, DEBIDO A QUE SE EXEDIO EL LIMITE DE MATERIAL PERMITIDO');
    ELSIF vTotal <> 0 AND vdisp= 'DISPONIBLE' AND vrestantes>0  THEN
        DBMS_OUTPUT.PUT_LINE('TIENES MULTAS NO PUEDES SACAR MATERIAL');
    ELSE
        DBMS_OUTPUT.PUT_LINE('ESTE MATERIAL NO ESTA DISPONIBLE O NO PUEDE SALIR DE LA BIBLIOTECA');
    END IF;
END;
/
